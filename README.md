...
- 마이페이지, 신고하기, 카테고리별 상품조회 완성 <br/>
거래상태, 찜기능 오늘 내로 완성예정


> 지금까지 완성된 부분
- 상품, 후기작성, 메인화면 상품조회, 카테고리 조회, 마이페이지(동네인증, 회원정보수정, 거래목록조회,후기조회), 
관리자 기능(공지작성, 신고처리), 신고하기 까지 완성
...
1. 상품 등록 시 동네인증 여부 검사
2. 상품 상태변경 요청 추가
3. 상품찜하기 취소요청 추가
4. 페이지 이동시 조회수 증가
5. 시큐리티 ADMIN 권한 추가 및 ADMIN 계정 생성
6. 요청시마다 SECURITY 적용하기
